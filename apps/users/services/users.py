...
class UserService:
    @staticmethod
    def create_user(user_data: UserCreateSerializer) -> Response:
        try:
    ...
    @staticmethod
    def update_user(user_id: int, user_data: UserUpdateSerializer) -> UserBaseSerializer:
        try:
            if Users.objects.get(id=user_id) is None:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
            update_data = {key: value for key, value in user_data.dict().items() if value is not None}
            Users.objects.update(id=user_id, **update_data)
    ...
    def delete_user(user_id: int) -> Response:
        try:
            Users.objects.delete(id=user_id)
